...
def test_the_policies_give_the_town_once_it_is_set(client):
    ...
    client.post(
        "/settings/site",
        data={"site_operator_locality": "Anytown, NY 12345"},
    )

    page = prose(client.get("/messaging-policy").text)

    assert "pump equipment in Anytown, NY 12345" in page
...
def test_the_policies_never_name_the_building(client):
    ...
    client.post(
        "/settings/site",
        data={"site_name": "123 Main St", "site_operator_locality": "Anytown, NY 12345"},
    )

    client.post("/logout")

    for path in ("/", "/contact", "/messaging-policy", "/privacy", "/login"):
        assert "123 Main St" not in client.get(path).text, path
...
def test_the_policy_stands_up_without_any_contact_details(client):
    ...
    """
    sign_in_as_admin(client)
    client.post("/settings/site", data={"site_name": "123 Main St"})
...
def test_contact_details_are_shown_when_there_are_some(client):
    ...
    client.post(
        "/settings/site",
        data={"site_name": "123 Main St", "site_contact_email": "pumps@example.com"},
    )
...
def test_a_sample_message_does_not_leak_the_building(client):
    ...
    assert "123 Main St" not in client.get("/messaging-policy").text
