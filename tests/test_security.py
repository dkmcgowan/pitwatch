...
def test_a_post_with_the_right_token_goes_through(client):
    ...
    client.post(
        "/settings/site",
        data={"site_name": "123 Main St", "csrf_token": token_from(client, "/settings")},
    )

    assert client.app.state.settings.site.name == "123 Main St"
...
def test_checking_the_token_does_not_eat_the_form(client):
    ...
    assert site.name == "123 Main St", "the handler saw an empty form"
